...
- Cross-module contracts must exist in the producing package before CLI wires against them. When CLI code depends on fields/exports another module is landing concurrently, prefer an `as unknown as {...}` cast with graceful undefined-handling (render '—'/0/no-output) over assuming the shape.
- AgentLedgerEntry (packages/core/src/types.ts, owned by runtime module) has no per-run diagnostic fields (verifyOutcome, refusalCount, quarantinedPaths, quarantineCommitHash) — only in-memory MissionModuleResult from a live `swarm mission` run can carry these. Past-mission history surfaces (e.g. `swarm missions`) structurally cannot show them.
- In `printResults()` (mission.ts) and in `missionCommand`'s own violations/branches/stranded/refused/followUps/quarantine/Report-note sections, each conditional block owns its **own unconditional leading `line()`/blank-line separator** regardless of what fired before it — sections never check a sibling's fired-state before adding their separator. Special-casing one block's separator based on another block's state (e.g. `if (refused.length === 0) line()`) breaks this convention. Confirmed by review 2026-08-21; do not "fix" spacing here without re-reading all sibling blocks first. <sub>`packages/cli/src/commands/mission.ts`</sub>
...
- ui.ts's LiveBoard has zero awareness of commit/branch/ownership/quarantine state — it only tracks module/state/activity/tokens/verifyOutcome/refusalCount rows. Live surfacing of quarantine info during a run (not just the final report) would require a new AgentRow field wired through onProgress/paintRow.
...
- As of 2026-08-21, mission.ts's verifyOutcomeOf/refusalCountOf **and** newly-added quarantinedPathsOf/quarantineCommitHashOf all read MissionModuleResult fields via the same `as unknown as {...}` cast pattern, because core's `MissionModuleResult` (packages/core/src/mission/run.ts, ~lines 175-195) does not declare verifyOutcome/refusalCount/quarantinedPaths/quarantineCommitHash. A concurrent mission-module change is expected to add these fields — confirm actual field names/shape land unchanged before removing any of these casts.
- packages/cli/src/commands/mission.test.ts's formatVerify/formatRefusals tests were loosened (2026-08-21) from exact literal-string assertions to substring/structural checks, specifically to survive future wording tweaks — this change was not contested by review and stands. New quarantinedPathsOf/quarantineCommitHashOf tests mirror this same style but only cover the accessors — no test exercises missionCommand's actual printed quarantine block (path truncation, hash formatting, message text) as of 2026-08-21.
...
- `packages/cli/src/commands/mission.ts` — largest handler; wires runMission callbacks into LiveBoard; renders verifyOutcome/refusalCount and (new, 2026-08-21) quarantinedPaths/quarantineCommitHash in printResults()/report sections. Ownership-violation and quarantine blocks are display-only, read MissionModuleResult verbatim, and sit as sibling blocks each with its own unconditional leading line(). Pure helpers (verifyOutcomeOf, refusalCountOf, formatVerify, formatRefusals, quarantinedPathsOf, quarantineCommitHashOf) unit-tested in mission.test.ts.
