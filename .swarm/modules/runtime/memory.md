...
- A tool_result only counts as a refusal (agent.tool_result.refused=true, rolled into AgentOutcome.refusalCount by collectAgent()) when is_error===true AND its content text matches a REFUSAL_PATTERNS phrase; an ordinary tool failure is never counted. refusalCount is always a number, never undefined. <sub>`packages/core/src/runtime/stream-json.ts`, `packages/core/src/runtime/collect.ts`</sub>
...
- checkStaleBuild's tsbuildinfo path is hardcoded as `<packageRoot>/tsconfig.tsbuildinfo`, derived from TS's default computation for this repo's current tsconfig layout — never observed from an actual build run. If any package's rootDir/outDir/tsBuildInfoFile setup changes, this silently reports perpetual false staleness. <sub>`packages/core/src/update/stale-build.ts`</sub>
- REFUSAL_PATTERNS (stream-json.ts) includes /permission denied/i and /not been granted/i alongside the one phrase actually observed in a live sandbox ('requires approval'). The first two are unconfirmed guesses; 'permission denied' in particular can be emitted by an ordinary OS-level Bash error (chmod, protected file) unrelated to sandbox approval gating, and would inflate refusalCount. Flagged in review, not yet fixed. <sub>`packages/core/src/runtime/stream-json.ts`</sub>
- AgentLedgerEntry (types.ts) now carries verifyOutcome/refusalCount, but MissionModuleResult (packages/core/src/mission/run.ts, outside this module's boundary) is the type the mission-layer verify loop and CLI display actually read per-module. Wiring ledger→result fields is the mission module's job, not done here.
...
_Last touched 2026-08-21: added refusal detection (stream-json.ts REFUSAL_PATTERNS) + AgentOutcome.refusalCount (collect.ts) + AgentLedgerEntry.verifyOutcome/refusalCount (types.ts) for the mission-layer verify loop / refusal-visibility feature. Confirmed via code review only that spec.resume+spec.prompt already support the fix-and-report-again call shape — no runtime code change needed for that half. Still NOT run through `npm test`/`npm run build` in this sandbox (node_modules symlink blocks all real command execution here) — verify before relying further. MissionModuleResult in mission/run.ts still needs matching fields wired by the mission module._
